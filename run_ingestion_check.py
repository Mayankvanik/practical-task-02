...
def run_pipeline(pdf_path: Path, recreate_collection: bool = False, skip_index: bool = False) -> dict:
    """
    ...
    if not skip_index:
        ...
        logger.info("━━━ Step 7/7 — Skipping Qdrant Indexing (--skip-index flag used)")
        summary["upserted"] = 0
...
def main():
    ...
    parser.add_argument(
        "--skip-index",
        action="store_true",
        help="Skip embedding and Qdrant indexing (useful for testing extraction)",
    )
    ...
    summary = run_pipeline(pdf_path=pdf_path, recreate_collection=args.recreate, skip_index=args.skip_index)
